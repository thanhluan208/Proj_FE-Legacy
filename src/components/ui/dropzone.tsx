	...
	({ className, children, ...props }, forwardedRef) => {
		...
		return (
			...
			<div
				...
				aria-label="dropzone"
				className={cn(
					"flex flex-col items-center justify-center space-y-4 border-2 border-gray-300 border-dashed rounded-md transition-colors hover:border-gray-400 focus-within:ring-2 focus-within:ring-primary focus-within:border-transparent",
...
interface DropZoneFileListProps
	extends React.OlHTMLAttributes<HTMLDivElement> {}

const DropzoneFileList = forwardRef<HTMLDivElement, DropZoneFileListProps>(
	(props, ref) => {
		const context = useDropzoneContext()
		if (!context) {
		...
		return (
			<div>
				<label
					...
					{...props}
					className={cn(
						"cursor-pointer rounded-sm w-full bg-secondary h-[183px] font-medium ring-offset-background transition-colors focus-within:outline-none has-[input:focus-visible]:ring-2 has-[input:focus-visible]:ring-ring has-[input:focus-visible]:ring-offset-2",
						className
					)}
				>
					{children}
